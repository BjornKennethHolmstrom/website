...
/**
 * Generate PDF for The Adaptation Bottleneck (Paper XV)
 *
 * Usage: node scripts/generate-adaptation-bottleneck-pdf.js [language]
 * Example: node scripts/generate-adaptation-bottleneck-pdf.js en
 * Example: node scripts/generate-adaptation-bottleneck-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/adaptation-bottleneck/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `adaptation-bottleneck${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/adaptation-bottleneck-cover.png');
...
const metadata = {
	en: {
		...
		description: 'Treats the Sense–Learn–Execute triad as simultaneous claims on finite processing capacity. Shows that effective adaptive throughput is gated by the slowest stage. Identifies three backlogs — information, innovation, reality — and derives design implications for functionally separating the loop\'s legs. The dynamic dual of Paper V.',
		author: 'Björn Kenneth Holmström',
		date: 'June 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/adaptation-bottleneck',
		seriesNote: 'Paper XV in the Governance as Engineering series',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	},
	sv: {
		title: 'Adaptationsflaskhalsen',
		subtitle: 'Genomströmningsbegränsningar i Sense–Learn–Execute-loopen',
		description: 'Behandlar Sense–Learn–Execute-triaden som samtidiga anspråk på ändlig processorkapacitet. Visar att effektiv adaptiv genomströmning begränsas av det långsammaste steget. Identifierar tre eftersläpningar — information, innovation, verklighet — och härleder designimplikationer för funktionell separation av loopens ben. Den dynamiska dualen till Rapport V.',
		author: 'Björn Kenneth Holmström',
		date: 'Juni 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/adaptation-bottleneck',
		seriesNote: 'Rapport XV i serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	...
	{ file: '01-part-1',   titleEn: 'Part I: The Problem — The Triad Was Never Costed',  titleSv: 'Del I: Problemet — Triaden var aldrig kostnadsberäknad' },
	{ file: '02-part-2',   titleEn: 'Part II: Formal Framework',                         titleSv: 'Del II: Formellt ramverk' },
	{ file: '03-part-3',   titleEn: 'Part III: The Three Backlogs, and Where They Are Documented', titleSv: 'Del III: De tre eftersläpningarna, och var de är dokumenterade' },
	{ file: '04-part-4',   titleEn: 'Part IV: Boundaries and Objections',                titleSv: 'Del IV: Gränser och invändningar' },
	{ file: '05-part-5',   titleEn: 'Part V: Simulation',                                titleSv: 'Del V: Simulering' },
	{ file: '06-part-6',   titleEn: 'Part VI: Design Implications, and What the Series Does Next', titleSv: 'Del VI: Designimplikationer, och vad serien gör härnäst' },
	{ file: '07-appendix-a', titleEn: 'Appendix A: Formal Derivations',         titleSv: 'Appendix A: Formella härledningar', isAppendix: true },
	{ file: '07-appendix-b', titleEn: 'Appendix B: Simulation Specification',        titleSv: 'Appendix B: Simuleringsspecifikation', isAppendix: true },
];
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/adaptation-bottleneck', imagePath),
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-adaptation-bottleneck-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series XV' : 'Artikel · Serie XV'}</span>
				</div>
			`,
			footerTemplate: `
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for The Adaptation Bottleneck (Paper XV)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-adaptation-bottleneck-${LANGUAGE}.html`);
