...
/**
 * Generate PDF from The Alchemist's Dilemma markdown files
 * 
 * Usage: node scripts/generate-alchemists-pdf.js [language]
 * Example: node scripts/generate-alchemists-pdf.js en
 * Example: node scripts/generate-alchemists-pdf.js sv
 * 
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/essays/alchemists-dilemma');
const OUTPUT_DIR = path.join(__dirname, '../static/essays');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `alchemists-dilemma${suffix}.pdf`);
...
function readMarkdownFiles() {
	...
	const parts = [];
	for (let i = 1; i <= 3; i++) {
		const filename = `part${i}${suffix}.md`;
...
`;
	...
	parts.forEach((part, index) => {
		if (index > 0) {
			html += `\n<div class="part-divider"><h1>Part ${part.number}</h1></div>\n`;
		}
