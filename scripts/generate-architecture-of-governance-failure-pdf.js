...
/**
 * Generate PDF for "The Architecture of Governance Failure" working paper
 *
 * Usage: node scripts/generate-architecture-of-governance-failure-pdf.js
 *
 * Requirements:
...
// ── Configuration ────────────────────────────────────────────────────────────
const LANGUAGE = 'en';
const PAPER_SLUG = 'architecture-of-governance-failure';
const INPUT_DIR = path.join(__dirname, `../src/routes/working-papers/${PAPER_SLUG}/sections`);
const OUTPUT_DIR = path.join(__dirname, `../static/working-papers`);
const OUTPUT_FILE = path.join(OUTPUT_DIR, `${PAPER_SLUG}.pdf`);
const COVER_IMAGE = path.join(__dirname, `../static/working-papers/images/${PAPER_SLUG}-cover.png`);
...
const meta = {
    ...
    description: "A unified framework diagnosing why governance systems become blind to their own fragility. Connects Ashby's Law of Requisite Variety, Goodhart's Law, and the coordination failure tax into a single architecture of institutional collapse, and derives the architectural prerequisites for adaptive governance.",
    author: "Björn Kenneth Holmström",
    date: "May 2026",
    url: "https://bjornkennethholmstrom.org/working-papers/architecture-of-governance-failure",
    license: "Creative Commons Attribution-ShareAlike 4.0 International",
    type: "Working Paper · Synthesis"
};

// ── Section definitions ──────────────────────────────────────────────────────
const sections = [
    { file: 'preface',  title: 'Preface',                                               isAppendix: false },
    { file: 'part-1',   title: 'Part I: The Pattern Nobody Named',                     isAppendix: false },
    { file: 'part-2',   title: 'Part II: Four Ways the Channel Breaks',               isAppendix: false },
    { file: 'part-3',   title: 'Part III: Why Reforms Disappoint',                    isAppendix: false },
    { file: 'part-4',   title: 'Part IV: What Requisite Governance Looks Like — and Where to Begin', isAppendix: false },
];

// ── CSS (identical to other working papers) ───────────────────────────────────
const pdfStyles = `
<style>
	@page {
		size: A4;
...
function readMarkdownFiles() {
    console.log(`\n📖 Reading paper sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
    ...
    const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-${PAPER_SLUG}.html`);
...
async function main() {
    try {
        ...
        if (process.env.DEBUG) {
            const debugFile = path.join(__dirname, `../debug-${PAPER_SLUG}.html`);
