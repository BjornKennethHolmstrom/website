...
/**
 * Generate PDF for Below the Factorization (Paper 0)
 *
 * Usage: node scripts/generate-below-the-factorization-pdf.js [language]
 * Example: node scripts/generate-below-the-factorization-pdf.js en
 * Example: node scripts/generate-below-the-factorization-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/below-the-factorization/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `below-the-factorization${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/below-the-factorization-cover.png');
...
const metadata = {
	en: {
		...
		description: 'The foundations paper of the Governance as Engineering series. Argues that factorization — the observation channel every later paper relies on — emerges from bounded representational capacity plus a temporal‑prediction objective, without action or viability pressure. A minimal model exhibits emergence, structured blindness, and non‑unique, symmetry‑broken selection across forty preregistered seeds.',
		author: 'Björn Kenneth Holmström',
		date: 'July 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/below-the-factorization',
		seriesNote: 'Paper 0 in the Governance as Engineering series',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	},
	sv: {
		title: 'Under faktoriseringen',
		subtitle: 'Begränsad prediktion och observationskanalens ursprung',
		description: 'Grundartikeln i serien Styrning som ingenjörskonst. Argumenterar för att faktorisering — observationskanalen varje senare artikel förlitar sig på — uppstår ur begränsad representationskapacitet plus ett tidsprediktionsmål, utan krav på handling eller överlevnadstryck. En minimal modell uppvisar emergens, strukturerad blindhet och icke‑unikt, symmetribrutet urval över fyrtio förregistrerade frön.',
		author: 'Björn Kenneth Holmström',
		date: 'Juli 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/below-the-factorization',
		seriesNote: 'Artikel 0 i serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	...
	{ file: 'section-01',  titleEn: '1. The unexamined primitive',                                      titleSv: '1. Den oprövade primitiven' },
	{ file: 'section-02',  titleEn: '2. The two ingredients',                                           titleSv: '2. De två ingredienserna' },
	{ file: 'section-03',  titleEn: '3. The minimal model: factorization from bounded prediction',       titleSv: '3. Den minimala modellen: faktorisering från begränsad prediktion' },
	{ file: 'section-04',  titleEn: '4. Non‑uniqueness: the equivalence class',                         titleSv: '4. Icke‑unikhet: ekvivalensklassen' },
	{ file: 'section-05',  titleEn: '5. Pragmatic preference without ontological preference',           titleSv: '5. Pragmatisk preferens utan ontologisk preferens' },
	{ file: 'section-06',  titleEn: '6. What this re‑grounds',                                           titleSv: '6. Vad detta återgrundar' },
	{ file: 'section-07',  titleEn: '7. Conclusion',                                                     titleSv: '7. Slutsats' },
	{ file: 'section-08',  titleEn: '8. What this paper does not show',                                  titleSv: '8. Vad denna artikel inte visar' },
	{ file: 'appendix-a',  titleEn: 'Appendix A: Model, training, and outputs',                          titleSv: 'Appendix A: Modell, träning och utdata', isAppendix: true },
];
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/below-the-factorization', imagePath),
...
function readMarkdownFiles() {
	console.log(`\n📖 Reading Paper 0 sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-below-the-factorization-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series 0' : 'Arbetsdokument · Serie 0'}</span>
				</div>
			`,
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for Below the Factorization (Paper 0)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-below-the-factorization-${LANGUAGE}.html`);
