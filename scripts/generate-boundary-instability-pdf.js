...
/**
 * Generate PDF for The Boundary Instability Principle (Paper XVIII)
 *
 * Usage: node scripts/generate-boundary-instability-pdf.js [language]
 * Example: node scripts/generate-boundary-instability-pdf.js en
 * Example: node scripts/generate-boundary-instability-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/boundary-instability/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `boundary-instability${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/boundary-instability-cover.png');
...
const metadata = {
	en: {
		...
		description: 'Extends Paper XII by removing the exogeneity assumption on coupling structure. Proves a Non‑Factorizability Theorem, exhibits a reflexive boundary cycle, derives a Critical Learning Bandwidth that closes endogenously, and documents a failing early‑warning index. Reframes governance as maintaining a decomposability margin under the drift that learning induces.',
		author: 'Björn Kenneth Holmström',
		date: 'July 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/boundary-instability',
		seriesNote: 'Paper XVIII in the Governance as Engineering series',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	},
	sv: {
		title: 'Principen om gränsinstabilitet',
		subtitle: 'Reflexiv styrning och den endogena driften av nedbrytbarhet',
		description: 'Utvidgar Rapport XII genom att ta bort exogenitetsantagandet om kopplingsstruktur. Bevisar ett icke‑faktoriserbarhetsteorem, uppvisar en reflexiv gränscykel, härleder en kritisk inlärningsbandbredd som stängs endogent och dokumenterar ett tidigt varningsindex som misslyckas. Omformulerar styrning som att upprätthålla en nedbrytbarhetsmarginal under den drift som lärande inducerar.',
		author: 'Björn Kenneth Holmström',
		date: 'Juli 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/boundary-instability',
		seriesNote: 'Rapport XVIII i serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	...
	{ file: '01-section-01',  titleEn: '1. Introduction: From Fixed Boundaries to Reflexive Decompositions', titleSv: '1. Introduktion: Från fasta gränser till reflexiva dekompositioner' },
	{ file: '02-section-02',  titleEn: '2. The Non‑Factorizability Theorem',                                 titleSv: '2. Icke‑faktoriserbarhetsteoremet' },
	{ file: '03-section-03',  titleEn: '3. The Reflexive Boundary Cycle: A Minimal Model',                   titleSv: '3. Den reflexiva gränscykeln: En minimal modell' },
	{ file: '04-section-04',  titleEn: '4. The Critical Learning Bandwidth',                                 titleSv: '4. Den kritiska inlärningsbandbredden' },
	{ file: '05-section-05',  titleEn: '5. The Boundary Dissolution Index, Revised by Its Own Test',         titleSv: '5. Gränsupplösningsindexet, reviderat genom sitt eget test' },
	{ file: '06-section-06',  titleEn: '6. Design Principles for Meta‑Stable Governance',                    titleSv: '6. Designprinciper för metastabil styrning' },
	{ file: '07-section-07',  titleEn: '7. Implications and Structural Integration',                         titleSv: '7. Implikationer och strukturell integration' },
	{ file: '08-section-08',  titleEn: '8. Conclusion: The Meta‑Stable Governance Attractor',                titleSv: '8. Slutsats: Den metastabila styrningsattraktorn' },
	{ file: '09-section-09',  titleEn: '9. What This Paper Does Not Show',                                   titleSv: '9. Vad denna rapport inte visar' },
...
// ── CSS (12pt base, same as other recent papers) ─────────────────────────────
const pdfStyles = `
<style>
	@page {
		size: A4;
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/boundary-instability', imagePath),
...
function readMarkdownFiles() {
	console.log(`\n📖 Reading Paper XVIII sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-boundary-instability-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series XVIII' : 'Artikel · Serie XVIII'}</span>
				</div>
			`,
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for The Boundary Instability Principle (Paper XVIII)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-boundary-instability-${LANGUAGE}.html`);
