...
/**
 * Generate PDF for The Certification Floor (Paper XVII)
 *
 * Usage: node scripts/generate-certification-floor-pdf.js [language]
 * Example: node scripts/generate-certification-floor-pdf.js en
 * Example: node scripts/generate-certification-floor-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/certification-floor/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `certification-floor${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/certification-floor-cover.png');
...
const metadata = {
	en: {
		...
		date: 'July 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/certification-floor',
		seriesNote: 'Paper XVII in the Governance as Engineering series',
	...
	sv: {
		...
		author: 'Björn Kenneth Holmström',
		date: 'Juli 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/certification-floor',
		seriesNote: 'Rapport XVII i serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	...
	{ file: 'section-01',  titleEn: '1. The Tradeoff: Chain Length Against Adaptivity',                 titleSv: '1. Avvägningen: kedjelängd mot adaptivitet' },
	{ file: 'section-02',  titleEn: '2. The Deeper Object: The Certification / Processing Asymmetry',  titleSv: '2. Det djupare objektet: certifierings‑/bearbetningsasymmetrin' },
	{ file: 'section-03',  titleEn: '3. The Relocation Invariant',                                     titleSv: '3. Omlokaliseringsinvarianten' },
	{ file: 'section-04',  titleEn: '4. The Floor Mechanism: Irreducible World‑Certification',         titleSv: '4. Golvmekanismen: irreducibel världscertifiering' },
	{ file: 'section-05',  titleEn: '5. Not All Irreducible Links Are Equal: Discrete vs. Ambient',    titleSv: '5. Alla irreducibla länkar är inte lika: diskret vs. omgivande' },
	{ file: 'section-06',  titleEn: '6. The Scope Bound: World‑Coupled vs. Self‑Referential Coordination', titleSv: '6. Räckviddsgränsen: världskopplad vs. självreferentiell koordination' },
	{ file: 'section-07',  titleEn: '7. Design Implication',                                            titleSv: '7. Designimplikation' },
	{ file: 'section-08',  titleEn: '8. What This Does Not Show',                                       titleSv: '8. Vad detta inte visar' },
	{ file: 'section-09',  titleEn: '9. Method and Confidence',                                         titleSv: '9. Metod och konfidens' },
];

// ── CSS (same as previous papers, larger base size) ─────────────────────────
const pdfStyles = `
<style>
	@page {
		size: A4;
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/certification-floor', imagePath),
...
function readMarkdownFiles() {
	console.log(`\n📖 Reading Paper XVII sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-certification-floor-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series XVII' : 'Artikel · Serie XVII'}</span>
				</div>
			`,
			footerTemplate: `
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for The Certification Floor (Paper XVII)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-certification-floor-${LANGUAGE}.html`);
