...
/**
 * Generate PDF for Closure–Adaptation Dynamics (Paper XXVIII)
 *
 * Usage: node scripts/generate-closure-adaptation-pdf.js [language]
 * Example: node scripts/generate-closure-adaptation-pdf.js en
 * Example: node scripts/generate-closure-adaptation-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/closure-adaptation/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `closure-adaptation${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/closure-adaptation-cover.png');
...
const metadata = {
	en: {
		...
		description: 'Introduces a five‑variable dynamical model of institutional closure and adaptation under uncertainty. The model exhibits bistability, hysteresis, noise‑induced tipping, cascade collapse in coupled populations, and a critical constitutional permeability floor that prevents permanent closure. Formal results are reported as [R within model]; governance interpretations as [IP]. Paper XXVIII in the Governance as Engineering series.',
		author: 'Björn Kenneth Holmström',
		date: 'September 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/closure-adaptation',
		seriesNote: 'Paper XXVIII in the Governance as Engineering series',
	...
	sv: {
		...
		author: 'Björn Kenneth Holmström',
		date: 'September 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/closure-adaptation',
		seriesNote: 'Rapport XXVIII i serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	{ file: '00-abstract',                    titleEn: 'Abstract',                                                           titleSv: 'Sammanfattning' },
	{ file: '01-introduction',                titleEn: '1. Introduction',                                                    titleSv: '1. Introduktion' },
	{ file: '02-definition-interpretation',   titleEn: '2. Model Definition and Governance Interpretation',                   titleSv: '2. Modelldefinition och styrningstolkning' },
	{ file: '03-analytical-structure',        titleEn: '3. Analytical Structure: Fast–Slow Decomposition and Attractor Regimes', titleSv: '3. Analytisk struktur: snabb–långsam dekomposition och attraktorregimer' },
	{ file: '04-phase-diagram-and-robustness', titleEn: '4. Phase Diagram and Robustness of the Closure Trap',                titleSv: '4. Fasdiagram och robusthet hos slutenhetsfällan' },
	{ file: '05-stochastic-dynamics',         titleEn: '5. Stochastic Dynamics: Noise-Induced Tipping and Residual Fragility', titleSv: '5. Stokastisk dynamik: brusinducerad tippning och kvarvarande skörhet' },
	{ file: '06-coupled-populations',         titleEn: '6. Coupled Populations: Polarization and Cascade Collapse',          titleSv: '6. Kopplade populationer: polarisering och kaskadkollaps' },
	{ file: '07-governance-intervention',     titleEn: '7. Governance Intervention: Constitutional Permeability Floor',      titleSv: '7. Styrningsintervention: konstitutionellt permeabilitetsgolv' },
	{ file: '08-discussion',                  titleEn: '8. Discussion',                                                      titleSv: '8. Diskussion' },
	{ file: '09-conclusion',                  titleEn: '9. Conclusion',                                                      titleSv: '9. Slutsats' },
	{ file: '10-appendix-a',                  titleEn: 'Appendix A: Simulation and Reproducibility Notes',                  titleSv: 'Appendix A: Simulerings- och reproducerbarhetsanteckningar', isAppendix: true },
];

// ── CSS (12pt base, with overflow‑wrap fixes) ────────────────────────────────
const pdfStyles = `
<style>
	@page {
		size: A4;
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/closure-adaptation', imagePath),
...
function readMarkdownFiles() {
	console.log(`\n📖 Reading Paper XXVIII sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-closure-adaptation-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series XXVIII' : 'Artikel · Serie XXVIII'}</span>
				</div>
			`,
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for Closure–Adaptation Dynamics (Paper XXVIII)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-closure-adaptation-${LANGUAGE}.html`);
