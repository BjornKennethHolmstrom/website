...
/**
 * Generate PDF for The Cost of Returning (Paper XXVI)
 *
 * Usage: node scripts/generate-cost-of-returning-pdf.js [language]
 * Example: node scripts/generate-cost-of-returning-pdf.js en
 * Example: node scripts/generate-cost-of-returning-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/cost-of-returning/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `cost-of-returning${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/cost-of-returning-cover.png');
...
const metadata = {
	en: {
		...
		date: 'July 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/cost-of-returning',
		seriesNote: 'Paper XXVI in the Governance as Engineering series',
	...
	sv: {
		...
		author: 'Björn Kenneth Holmström',
		date: 'Juli 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/cost-of-returning',
		seriesNote: 'Rapport XXVI i serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	...
	{ file: 'section-01',  titleEn: '1. The placeholder in Paper X',                                    titleSv: '1. Platshållaren i artikel X' },
	{ file: 'section-02',  titleEn: '2. Retained competence and the homogeneous loop',                   titleSv: '2. Kvarhållen kompetens och den homogena loopen' },
	{ file: 'section-03',  titleEn: '3. The escape‑ladder theorem',                                     titleSv: '3. Flyktstege‑satsen' },
	{ file: 'section-04',  titleEn: '4. The tail–ladder phase diagram',                                 titleSv: '4. Svans–stege fasdiagrammet' },
	{ file: 'section-05',  titleEn: '5. Institutional time and the escape hazard',                       titleSv: '5. Institutionell tid och flyktfaran' },
	{ file: 'section-06',  titleEn: '6. Recovery ecology',                                              titleSv: '6. Återhämtningsekologi' },
	{ file: 'section-07',  titleEn: '7. Interpretation, limits, and the ledger',                         titleSv: '7. Tolkning, gränser och bokslutet' },
	{ file: 'section-08',  titleEn: '8. Conclusion',                                                    titleSv: '8. Slutsats' },
	{ file: 'appendix-a',  titleEn: 'Appendix A: Proofs and derivations',                               titleSv: 'Appendix A: Bevis och härledningar', isAppendix: true },
	{ file: 'appendix-b',  titleEn: 'Appendix B: Simulation record',                                    titleSv: 'Appendix B: Simuleringsprotokoll', isAppendix: true },
	{ file: 'appendix-c',  titleEn: 'Appendix C: Claim‑tier table',                                     titleSv: 'Appendix C: Tabell över påståendenivåer', isAppendix: true },
];
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/cost-of-returning', imagePath),
...
function readMarkdownFiles() {
	console.log(`\n📖 Reading Paper XXVI sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-cost-of-returning-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series XXVI' : 'Artikel · Serie XXVI'}</span>
				</div>
			`,
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for The Cost of Returning (Paper XXVI)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-cost-of-returning-${LANGUAGE}.html`);
