...
/**
 * Generate PDF for Why Diversity Resists Formalization (Paper XVI)
 *
 * Usage: node scripts/generate-diversity-formalization-pdf.js [language]
 * Example: node scripts/generate-diversity-formalization-pdf.js en
 * Example: node scripts/generate-diversity-formalization-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/why-diversity-resists-formalization/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `why-diversity-resists-formalization${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/why-diversity-resists-formalization-cover.png');
...
const metadata = {
	en: {
		...
		description: 'Four disciplines — control theory, evolutionary biology, institutional economics, and decision theory — independently converge on a single decay‑plus‑source‑term structure for the erosion of alternatives under optimization. Proposes source‑term locality as the corrected order parameter. Paper XVI in the Governance as Engineering series.',
		author: 'Björn Kenneth Holmström',
		date: 'June 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/why-diversity-resists-formalization',
		seriesNote: 'Paper XVI in the Governance as Engineering series',
...
const sections = [
	...
	{ file: 'section-1',    titleEn: '1. The Object That Keeps Moving',                   titleSv: '1. Objektet som flyttar sig' },
	{ file: 'section-2',    titleEn: '2. Method: Adversarial Decay as Evidence',          titleSv: '2. Metod: Adversarial Decay som evidens' },
	{ file: 'section-3',    titleEn: '3. The Four Self‑Contained Collapses',              titleSv: '3. De fyra fristående kollapserna' },
	{ file: 'section-4',    titleEn: '4. The Shared Pattern, and the Corrected Order Parameter', titleSv: '4. Det gemensamma mönstret och den korrigerade ordningsparametern' },
	{ file: 'section-5',    titleEn: '5. What This Does Not Show',                        titleSv: '5. Vad detta inte visar' },
	{ file: 'section-6',    titleEn: '6. The Residual',                                   titleSv: '6. Restposten' },
	{ file: 'appendix-a',   titleEn: 'Appendix A: Lens Prompts',                          titleSv: 'Appendix A: Lins‑prompter', isAppendix: true },
	{ file: 'appendix-b',   titleEn: 'Appendix B: Rejected Unifications',                titleSv: 'Appendix B: Förkastade unifieringar', isAppendix: true },
];
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/why-diversity-resists-formalization', imagePath),
...
function readMarkdownFiles() {
	console.log(`\n📖 Reading Paper XVI sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-diversity-formalization-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series XVI' : 'Artikel · Serie XVI'}</span>
				</div>
			`,
			footerTemplate: `
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for Why Diversity Resists Formalization (Paper XVI)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-diversity-formalization-${LANGUAGE}.html`);
