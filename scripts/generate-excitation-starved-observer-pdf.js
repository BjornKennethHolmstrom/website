...
/**
 * Generate PDF for The Observer You Cannot Afford to Excite (Paper XXIV)
 *
 * Usage: node scripts/generate-excitation-starved-observer-pdf.js [language]
 * Example: node scripts/generate-excitation-starved-observer-pdf.js en
 * Example: node scripts/generate-excitation-starved-observer-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/excitation-starved-observer/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `excitation-starved-observer${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/excitation-starved-observer-cover.png');
...
const metadata = {
	...
	sv: {
		...
		description: 'Frågar vad som förändras när ett diversitetsmått blir ett optimeringsmål snarare än en passiv sensor. En inlärd observatör allokerar upplösning till ofta besökta regioner och komprimerar periferin; optimering av en diversitetsproxy byggd på den observatören driver proxyn till sitt tak samtidigt som den förstör nåbar tillgång till perifera alternativ. Den blinda fläcken produceras och upprätthålls av den slutna loopen, och endast strukturell exponering — inte bredare utforskning — reparerar frånkopplingen. Rapport XXIV i serien Styrning som ingenjörskonst.',
		author: 'Björn Kenneth Holmström',
		date: 'Juli 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/excitation-starved-observer',
		seriesNote: 'Rapport XXIV i serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	...
	{ file: 'section-02',  titleEn: '2. Conceptual model',                                                titleSv: '2. Konceptuell modell' },
	{ file: 'section-03',  titleEn: '3. Experiment 1: a proxy that is a sensor but not an objective',    titleSv: '3. Experiment 1: en proxy som är en sensor men inte ett mål' },
	{ file: 'section-04',  titleEn: '4. Experiment 2: resolution and access cost as controlled variables', titleSv: '4. Experiment 2: upplösning och åtkomstkostnad som kontrollerade variabler' },
	{ file: 'section-05',  titleEn: '5. Experiment 3: the bias arises from the data',                    titleSv: '5. Experiment 3: biasen uppstår ur datan' },
	{ file: 'section-06',  titleEn: '6. The mechanism: an excitation-starved observer',                   titleSv: '6. Mekanismen: en exciteringssvulten observatör' },
	{ file: 'section-07',  titleEn: '7. A governance hypothesis',                                        titleSv: '7. En styrningshypotes' },
	{ file: 'section-08',  titleEn: '8. What did not survive',                                           titleSv: '8. Vad som inte överlevde' },
	{ file: 'section-09',  titleEn: '9. Limitations',                                                    titleSv: '9. Begränsningar' },
	{ file: 'section-10',  titleEn: '10. Future work',                                                   titleSv: '10. Framtida arbete' },
	{ file: 'section-11',  titleEn: '11. Conclusion',                                                    titleSv: '11. Slutsats' },
	{ file: 'appendix-a',  titleEn: 'Appendix A: Environment, agent, and hyperparameters',               titleSv: 'Appendix A: Miljö, agent och hyperparametrar', isAppendix: true },
	{ file: 'appendix-b',  titleEn: 'Appendix B: Reproducibility manifest',                              titleSv: 'Appendix B: Reproducerbarhetsmanifest', isAppendix: true },
	{ file: 'appendix-c',  titleEn: 'Appendix C: Statistical methods',                                   titleSv: 'Appendix C: Statistiska metoder', isAppendix: true },
];
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/excitation-starved-observer', imagePath),
...
function readMarkdownFiles() {
	console.log(`\n📖 Reading Paper XXIV sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-excitation-starved-observer-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series XXIV' : 'Artikel · Serie XXIV'}</span>
				</div>
			`,
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for The Observer You Cannot Afford to Excite (Paper XXIV)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-excitation-starved-observer-${LANGUAGE}.html`);
