...
/**
 * Generate PDF for the Netherlands Feasibility Debt report
 *
 * Usage: node scripts/generate-netherlands-report-pdf.js [language]
 * Example: node scripts/generate-netherlands-report-pdf.js en
 * Example: node scripts/generate-netherlands-report-pdf.js nl
 *
 * Requirements:
...
const REPORT_SLUG = 'netherlands-feasibility-debt';
...
const metadata = {
	en: {
		title: 'The Netherlands: The Feasibility Debt',
		subtitle: 'Commitment Before Durable Feasibility',
		description:
		...
		date: 'September 2026',
		url: 'https://bjornkennethholmstrom.org/reports/netherlands-feasibility-debt',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International',
		type: 'Country Report · Netherlands'
	},
	nl: {
		title: 'Nederland: De Haalbaarheidsschuld',
		subtitle: 'Toezegging vóór Duurzame Haalbaarheid',
		description:
...
// NOTE: the file name for section 5 contains a typo ('transtition') and this
// is preserved here deliberately to match the on-disk filenames.
const sections = [
	{ file: '00-executive-summary',               titleEn: 'Executive Summary',                                                     titleNl: 'Samenvatting',                                                          isAppendix: false },
	{ file: '01-feasibility-debt',                titleEn: '1. The Feasibility Debt',                                               titleNl: '1. De Haalbaarheidsschuld',                                             isAppendix: false },
	{ file: '02-structural-mechanisms',           titleEn: '2. The Feasibility Debt: Structural Mechanisms',                        titleNl: '2. De Haalbaarheidsschuld: Structurele Mechanismen',                    isAppendix: false },
	{ file: '03-building-recursive-feasibility',  titleEn: '3. What Building Recursive Feasibility Would Look Like',                titleNl: '3. Hoe het Bouwen van Recursieve Haalbaarheid eruit zou zien',          isAppendix: false },
	{ file: '04-political-immune-system',         titleEn: '4. The Political Immune System: What Keeps Producing Debt',             titleNl: '4. Het Politieke Immuunsysteem: Wat Schulden Blijft Produceren',        isAppendix: false },
	{ file: '05-transtition-architecture',        titleEn: '5. Working with the Grain: Transition Architecture for the Netherlands', titleNl: '5. Met de Stroom Mee: Transitiearchitectuur voor Nederland',            isAppendix: false },
	{ file: '06-first-step',                      titleEn: '6. A Concrete First Step: The Feasibility Balance Sheet',               titleNl: '6. Een Concrete Eerste Stap: De Haalbaarheidsbalans',                   isAppendix: false },
	{ file: '07-coda',                            titleEn: '7. Coda: The Makers of Land at the Limit of Promise',                   titleNl: '7. Coda: De Landmakers op de Grens van de Belofte',                    isAppendix: false },
	{ file: '08-appendix-a',                      titleEn: 'Appendix A: Value Systems and Policy Mindsets',                         titleNl: 'Bijlage A: Waardesystemen en beleidsmindset',                           isAppendix: true  },
	{ file: '08-appendix-b',                      titleEn: 'Appendix B: International and Domestic Analogues',                      titleNl: 'Bijlage B: Internationale en binnenlandse analogen',                    isAppendix: true  },
	{ file: '08-appendix-c',                      titleEn: 'Appendix C: The Governance as Engineering Connection',                  titleNl: 'Bijlage C: De verbinding met Governance as Engineering',                isAppendix: true  },
	{ file: '08-appendix-d',                      titleEn: 'Appendix D: Anticipated Objections',                                    titleNl: 'Bijlage D: Verwachte tegenwerpingen',                                   isAppendix: true  },
	{ file: '08-appendix-e',                      titleEn: 'Appendix E: About the Author and Method',                               titleNl: 'Bijlage E: Over de auteur en de methode',                              isAppendix: true  },
	{ file: '08-appendix-f',                      titleEn: 'Appendix F: Case File — Nitrogen and the Renewable Subsidy Cohort',     titleNl: 'Bijlage F: Dossier — Stikstof en het cohort hernieuwbare subsidies',    isAppendix: true  },
	{ file: '08-appendix-g',                      titleEn: 'Appendix G: Case File — Schiphol and Lelystad',                         titleNl: 'Bijlage G: Dossier — Schiphol en Lelystad',                             isAppendix: true  }
];
...
<style>
	...
	/* Standalone italic paragraphs (figure captions written as *text*) */
	.caption {
		display: block;
		font-size: 9.5pt;
		color: #555;
...
// In this report, every $ followed by a digit is a currency amount
// (US$539, US$200, US$5 billion, $40 billion, etc.).
// Markdown renders \$ as a literal $, so escaping is transparent.
function escapeCurrencyDollars(content) {
	return content.replace(/\$(\d)/g, (_, digit) => `\\$${digit}`);
}
...
function renderMathInMarkdown(content) {
	...
	// 2. Inline math: $ ... $ and \( ... \)
	// Lookbehind rejects both '$' and '\$' so escaped currency dollars are
	// not treated as inline-math delimiters.
...
function readMarkdownFiles() {
	...
	for (const section of sections) {
		...
		const title = LANGUAGE === 'nl' ? section.titleNl : section.titleEn;
...
`;
	...
	sections.forEach((section, index) => {
		...
		// 0. Escape currency dollars so they are not mistaken for math delimiters
		const escaped = escapeCurrencyDollars(section.content);

		// 1. Render math placeholders
		const { content: mathProcessed, blocks } = renderMathInMarkdown(escaped);
		...
		// 5. Promote only truly standalone <em> paragraphs to .caption
...
async function generatePDF(html) {
	...
	try {
		...
		// Give fonts (KaTeX + body) a moment to fully load before rendering
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for Netherlands Feasibility Debt report...\n');
