...
/**
 * Generate PDF for The Political Economy of Requisite Governance (Paper IX)
 *
 * Usage: node scripts/generate-political-economy-pdf.js [language]
 * Example: node scripts/generate-political-economy-pdf.js en
 * Example: node scripts/generate-political-economy-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/political-economy-of-requisite-governance/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `political-economy-of-requisite-governance${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/political-economy-requisite-governance-cover.png');
...
const metadata = {
	en: {
		...
		description: 'Models the transition to requisite governance as a contested control problem between reform coalitions and incumbent controllers. Introduces the transition variety ratio Ω, formalizes three structural traps, and derives design principles for transition mechanisms including buy‑out protocols, sunset‑coupled bypasses, and protected experimental spaces. Includes historical calibration and three simulations.',
		author: 'Björn Kenneth Holmström',
		date: 'June 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/political-economy-of-requisite-governance',
		seriesNote: 'Paper IX in the Governance as Engineering series',
	...
	sv: {
		...
		description: 'Modellerar övergången till nödvändig styrning som ett kontrollproblem mellan reformkoalitioner och etablerade kontrollanter. Introducerar övergångsvarietetskvoten Ω, formaliserar tre strukturella fällor och härleder konstruktionsprinciper för övergångsmekanismer, inklusive utköpsprotokoll, solnedgångskopplade bypassar och skyddade experimentutrymmen. Historisk kalibrering och simuleringar ingår.',
		author: 'Björn Kenneth Holmström',
		date: 'Juni 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/political-economy-of-requisite-governance',
		seriesNote: 'Rapport IX i serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	{ file: 'part-1', titleEn: 'Part I: From Qualitative Diagnosis to Formal Model',               titleSv: 'Del I: Från kvalitativ diagnos till formell modell',                 isAppendix: false },
	{ file: 'part-2', titleEn: 'Part II: Modeling the Transition as a Contested Control Problem',  titleSv: 'Del II: Modellering av övergången som ett kontrollerat kontrollproblem', isAppendix: false },
	{ file: 'part-3', titleEn: 'Part III: The Three Structural Traps',                             titleSv: 'Del III: De tre strukturella fällorna',                              isAppendix: false },
	{ file: 'part-4', titleEn: 'Part IV: Design Principles for Transition Mechanisms',             titleSv: 'Del IV: Konstruktionsprinciper för övergångsmekanismer',            isAppendix: false },
	{ file: 'part-5', titleEn: 'Part V: Dynamic Model: Transition Bandwidth',                      titleSv: 'Del V: Dynamisk modell: övergångsbandbredd',                        isAppendix: false },
	{ file: 'part-6', titleEn: 'Part VI: Simulation Architecture',                                 titleSv: 'Del VI: Simuleringsarkitektur',                                     isAppendix: false },
	{ file: 'part-7', titleEn: 'Part VII: Limitations and Open Questions',                         titleSv: 'Del VII: Begränsningar och öppna frågor',                           isAppendix: false },
	{ file: 'part-8', titleEn: 'Part VIII: Conclusion',                                            titleSv: 'Del VIII: Slutsats',                                                isAppendix: false },
	{ file: 'appendix-a', titleEn: 'Appendix A: Heuristic Derivation of the Transition Variety Ratio Ω', titleSv: 'Appendix A: Heuristisk härledning av övergångsvarietetskvoten Ω', isAppendix: true },
	{ file: 'appendix-b', titleEn: 'Appendix B: The Bypass‑Trap Dynamical System',                 titleSv: 'Appendix B: Förbikopplingsfällans dynamiska system',                isAppendix: true },
	{ file: 'appendix-c', titleEn: 'Appendix C: Historical Calibration of Buy‑Out Effectiveness', titleSv: 'Appendix C: Historisk kalibrering av utköpseffektivitet',        isAppendix: true },
];

// ── CSS (identical in logic to original, only series references updated) ─────
const pdfStyles = `
<style>
	@page {
		size: A4;
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/political-economy-of-requisite-governance', imagePath),
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-political-economy-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series IX' : 'Artikel · Serie IX'}</span>
				</div>
			`,
			footerTemplate: `
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for Political Economy of Requisite Governance (Paper IX)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-political-economy-${LANGUAGE}.html`);
