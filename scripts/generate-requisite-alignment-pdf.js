...
/**
 * Generate PDF for Requisite Alignment (Paper XXVII)
 *
 * Usage: node scripts/generate-requisite-alignment-pdf.js [language]
 * Example: node scripts/generate-requisite-alignment-pdf.js en
 * Example: node scripts/generate-requisite-alignment-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/requisite-alignment/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `requisite-alignment${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/requisite-alignment-cover.png');
...
const metadata = {
	en: {
		...
		description: 'Asks whether dimensional sufficiency is enough. In a single exactly‑solved partially‑observed control problem, a fully informative feedback signal that evaluates a systematically wrong target loses substantial decision value — and two displacements of identical information content but different geometric structure impose different, oppositely‑ordered costs. The result isolates a second constraint orthogonal to requisite variety: the distinction an observable resolves must be aligned with the distinction the controller\'s action turns on, and alignment has geometry. Paper XXVII in the Governance as Engineering series.',
		author: 'Björn Kenneth Holmström',
		date: 'July 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/requisite-alignment',
		seriesNote: 'Paper XXVII in the Governance as Engineering series',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	},
	sv: {
		title: 'Nödvändig inriktning',
		subtitle: 'Geometrin hos informativ återkoppling i ett löst kontrollproblem',
		description: 'Frågar om dimensionell tillräcklighet är nog. I ett enda exakt löst partiellt observerbart kontrollproblem förlorar en fullt informativ återkopplingssignal som utvärderar ett systematiskt fel mål betydande beslutsvärde — och två förskjutningar med identiskt informationsinnehåll men olika geometrisk struktur medför olika, motsatt ordnade, kostnader. Resultatet isolerar en andra begränsning ortogonal mot nödvändig variation: den distinktion en observerbar storhet upplöser måste vara inriktad mot den distinktion regulators handling beror på, och inriktning har geometri. Rapport XXVII i serien Styrning som ingenjörskonst.',
		author: 'Björn Kenneth Holmström',
		date: 'Juli 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/requisite-alignment',
		seriesNote: 'Rapport XXVII i serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	...
	{ file: 'section-02',  titleEn: '2. Relation to the series: a refinement of the synthesis',            titleSv: '2. Relation till serien: en förfining av syntesen' },
	{ file: 'section-03',  titleEn: '3. Methods',                                                         titleSv: '3. Metoder' },
	{ file: 'section-04',  titleEn: '4. Results',                                                         titleSv: '4. Resultat' },
	{ file: 'section-05',  titleEn: '5. Discussion',                                                      titleSv: '5. Diskussion' },
	{ file: 'references',  titleEn: 'References',                                                         titleSv: 'Referenser' },
	{ file: 'appendix-a',  titleEn: 'Appendix A: Reproducibility',                                        titleSv: 'Appendix A: Reproducerbarhet', isAppendix: true },
];
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/requisite-alignment', imagePath),
...
function readMarkdownFiles() {
	console.log(`\n📖 Reading Paper XXVII sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-requisite-alignment-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series XXVII' : 'Artikel · Serie XXVII'}</span>
				</div>
			`,
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for Requisite Alignment (Paper XXVII)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-requisite-alignment-${LANGUAGE}.html`);
