...
/**
 * Generate PDF for Self III: The Operator
 *
 * Usage: node scripts/generate-self-operator-pdf.js [language]
 * Example: node scripts/generate-self-operator-pdf.js en
 * Example: node scripts/generate-self-operator-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/self-operator/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `self-operator${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/self-operator-cover.png');
...
const metadata = {
	en: {
		...
		description: 'Closes the seam between the Governance as Engineering series and the Self papers. Derives Inherited Unobservability: an institution\'s capacity to perceive interior dimensions of the governed is upper-bounded by the perceptual capacity of the human operators through whom those dimensions must pass. Couples operator self-opacity to distortion in estimating others\' interior states. Identifies the operator‑seeded legitimacy spiral. The final paper in the Self trilogy.',
		author: 'Björn Kenneth Holmström',
		date: 'June 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/self-operator',
		seriesNote: 'Self III — Companion to the Governance as Engineering series',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	},
	sv: {
		title: 'Operatören',
		subtitle: 'Hur institutioner ärver sina noders begränsningar',
		description: 'Stänger sömmen mellan serien Styrning som ingenjörskonst och Själv‑artiklarna. Härleder Ärvd oobserverbarhet: institutionens förmåga att uppfatta de styrdas inre dimensioner begränsas uppåt av den perceptuella kapaciteten hos de mänskliga operatörer genom vilka dessa dimensioner måste passera. Kopplar operatörens självopacitet till förvrängning i uppskattningen av andras inre tillstånd. Identifierar den operatörssådda legitimitetsnedåtgående spiralen. Den avslutande rapporten i Själv‑trilogin.',
		author: 'Björn Kenneth Holmström',
		date: 'Juni 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/self-operator',
		seriesNote: 'Själv III — Kompanjon till serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	...
	{ file: '01-part-0',   titleEn: 'Part 0: The Operator Seam',             titleSv: 'Del 0: Operatörssömmen' },
	{ file: '02-part-1',   titleEn: 'Part I: Inherited Unobservability',    titleSv: 'Del I: Ärvd oobserverbarhet' },
	{ file: '03-part-2',   titleEn: 'Part II: One State Vector',            titleSv: 'Del II: En tillståndsvektor' },
	{ file: '04-part-3',   titleEn: 'Part III: The Self-to-Other Distortion', titleSv: 'Del III: Själv‑till‑andra‑förvrängningen' },
	{ file: '05-part-4',   titleEn: 'Part IV: The Operator-Seeded Legitimacy Spiral', titleSv: 'Del IV: Den operatörssådda legitimitetsnedåtgående spiralen' },
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/self-operator', imagePath),
...
function readMarkdownFiles() {
	console.log(`\n📖 Reading Self III sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-self-operator-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Self III' : 'Artikel · Själv III'}</span>
				</div>
			`,
			footerTemplate: `
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for Self III: The Operator...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-self-operator-${LANGUAGE}.html`);
