...
/**
 * Generate PDF for When to Stop Learning, and When to Stop (Paper XXI)
 *
 * Usage: node scripts/generate-when-to-stop-learning-pdf.js [language]
 * Example: node scripts/generate-when-to-stop-learning-pdf.js en
 * Example: node scripts/generate-when-to-stop-learning-pdf.js sv
 *
 * Requirements:
...
const INPUT_DIR = path.join(__dirname, '../src/routes/working-papers/when-to-stop-learning/sections');
const OUTPUT_DIR = path.join(__dirname, '../static/working-papers');
const OUTPUT_FILE = path.join(OUTPUT_DIR, `when-to-stop-learning${suffix}.pdf`);
const COVER_IMAGE = path.join(__dirname, '../static/working-papers/images/when-to-stop-learning-cover.png');
...
const metadata = {
	en: {
		...
		description: 'Traces the lifecycle of a governance architecture. Three separations organize it: learning is not adaptation — a registered minimal model shows the map improving while the coupling degrades; meta‑learning is not free — a bounded hierarchy of learning rules must close on invariants; and persistence is not the goal — the terminal adaptive act may be to end or transfer rather than to continue. Paper XXI in the Governance as Engineering series.',
		author: 'Björn Kenneth Holmström',
		date: 'July 2026',
		url: 'https://bjornkennethholmstrom.org/working-papers/when-to-stop-learning',
		seriesNote: 'Paper XXI in the Governance as Engineering series',
	...
	sv: {
		...
		description: 'Spårar livscykeln för en styrningsarkitektur. Tre separationer organiserar den: lärande är inte anpassning — en registrerad minimal modell visar kartan förbättras medan kopplingen försämras; meta‑lärande är inte fritt — en begränsad hierarki av inlärningsregler måste stängas på invarianter; och beständighet är inte målet — den terminala adaptiva handlingen kan vara att avsluta eller överföra snarare än att fortsätta. Rapport XXI i serien Styrning som ingenjörskonst.',
		author: 'Björn Kenneth Holmström',
		date: 'Juli 2026',
		url: 'https://bjornkennethholmstrom.org/sv/working-papers/when-to-stop-learning',
		seriesNote: 'Rapport XXI i serien Styrning som ingenjörskonst',
		license: 'Creative Commons Attribution-ShareAlike 4.0 International'
	}
};
...
const sections = [
	...
	{ file: 'section-01',  titleEn: '1. The lifecycle question',                                      titleSv: '1. Livscykelfrågan' },
	{ file: 'section-02',  titleEn: '2. Learning is not adaptation',                                   titleSv: '2. Lärande är inte anpassning' },
	{ file: 'section-03',  titleEn: '3. A model where learning breaks coupling',                      titleSv: '3. En modell där lärande bryter kopplingen' },
	{ file: 'section-04',  titleEn: '4. Meta‑learning must close',                                    titleSv: '4. Meta‑lärande måste stängas' },
	{ file: 'section-05',  titleEn: '5. Closure requires invariants',                                 titleSv: '5. Stängning kräver invarianter' },
	{ file: 'section-06',  titleEn: '6. Persistence is not the goal — sunsetting',                    titleSv: '6. Beständighet är inte målet — avveckling' },
	{ file: 'section-07',  titleEn: '7. The unifying structure',                                      titleSv: '7. Den förenande strukturen' },
	{ file: 'section-08',  titleEn: '8. What this re‑grounds, and what it does not show',              titleSv: '8. Vad detta återgrundar, och vad det inte visar' },
	{ file: 'section-09',  titleEn: '9. Method and confidence',                                        titleSv: '9. Metod och konfidens' },
	{ file: 'appendix-a',  titleEn: 'Appendix A: The learning/adaptation model',                      titleSv: 'Appendix A: Modellen för lärande/anpassning', isAppendix: true },
];
...
function processMarkdownImagesSync(content) {
	...
	while ((match = imageRegex.exec(content)) !== null) {
		...
		const possiblePaths = [
			...
			path.join(__dirname, '../src/routes/working-papers/when-to-stop-learning', imagePath),
...
function readMarkdownFiles() {
	console.log(`\n📖 Reading Paper XXI sections for language: ${LANGUAGE}`);
...
async function generatePDF(html) {
	...
	const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-when-to-stop-learning-${LANGUAGE}.html`);
		...
		await page.pdf({
			...
			headerTemplate: `
				<div style="font-size: 9pt; color: #666; width: 100%; margin: 0 2cm;">
					...
					<span style="float: right;">${LANGUAGE === 'en' ? 'Working paper · Series XXI' : 'Artikel · Serie XXI'}</span>
				</div>
			`,
...
async function main() {
	try {
		console.log('📄 Starting PDF generation for When to Stop Learning, and When to Stop (Paper XXI)...\n');
		...
		if (process.env.DEBUG) {
			const debugFile = path.join(__dirname, `../debug-when-to-stop-learning-${LANGUAGE}.html`);
