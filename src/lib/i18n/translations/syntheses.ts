...
export const synthesesTranslations = {
  en: {
    ...
    digestTitle: 'The series in twenty-nine findings',
    papersDigest: [
     ...
     { ...papersEn.XXVIII, digest: 'A five‑variable dynamical model of institutional closure and adaptation under uncertainty. The same environment supports both an open, high‑trust, exploratory regime and a closed, low‑trust, non‑adaptive one — separated by history, not by external conditions. Collapse thresholds are higher than recovery thresholds, so systems that close need substantially safer conditions to reopen. Under high stakes and perceptual noise, a residual probability of noise‑induced tipping remains. In coupled populations, closure in one can cascade to another through shared uncertainty. A constitutional minimum on boundary permeability — held above a critical floor — prevents permanent closure by preserving the trust–exploration loop through the crisis.' },
     { href: '/essays/the-optimum-that-depended-on-the-ruler', label: "Note — The Optimum That Depended on the Ruler", digest: 'A computational exploration that began as a metaphor and produced a clean, pre‑registered interior optimum for adaptive escalation under subsidiarity — then dissolved under a robustness check that changed the definition of coordination capacity from event‑count‑based to magnitude‑based. The surviving findings are narrower but sharper: subsidiarity reduces coordination‑layer burden only when the receiving level has real capacity (delegation without capacity is worse than no delegation), the three architectures fail with qualitatively different signatures under the same stress, and match‑rate is pure arithmetic, not emergent intelligence. The lesson is that "coordination capacity" is not a self‑evident quantity but a modelling choice that can reverse the verdict.' },
    ],
    synthesesItems: [
      {
        slug: 'we-see-you',
  ...
  sv: {
    ...
    digestTitle: 'Serien i tjugonio resultat',
    papersDigest: [
      ...
      { href: '/essays/when-sensing-doesnt-help', label: 'Notis — när avkänning inte hjälper', digest: 'En föreslagen utvidgning — att ett system måste både observera och värdera en dold resiliensvariabel för att hindra dess framtida livskraftiga mängd från att kollapsa — överlevde tre simuleringar och producerade en ren, förhandsregistrerad interaktion, och föll sedan isär när den konkurrerande förklaringen till slut modellerades rättvist. En regulator som bara känner till störningsstatistiken kan hålla en stående buffert och bevara resiliens utan att alls observera tillståndet; och där återhämtning är hastighetsbegränsad slår den buffringen reaktiv reparation. Vad som återstår är snävare och mer klassiskt: buffra den förutsägbara störningen, övervaka de tryck som omdimensionerar bufferten, och behandla tillståndsövervakning som en källa till effektivitet och modellvalidering snarare än robusthet. Behållet som ett negativt resultat, eftersom sättet det upplöstes på är den disciplin som resten av serien bygger på.' },
       ...
       { ...papersSv.XXVIII, digest: 'En femvariabel dynamisk modell av institutionell slutenhet och anpassning under osäkerhet. Samma miljö kan bära både en öppen regim med hög tillit och utforskning och en sluten, lågtillitsregim utan anpassning — åtskilda av historik snarare än av yttre villkor. Tröskeln för kollaps är högre än tröskeln för återhämtning, så system som slutit sig behöver väsentligt tryggare förhållanden för att öppna sig igen. Vid höga insatser och perceptuellt brus kvarstår en liten sannolikhet för brusinducerad tippning. I kopplade populationer kan slutenhet i en grupp sprida sig till en annan via gemensam osäkerhet. Ett konstitutionellt golv för gränspermeabilitet — hållet över ett kritiskt värde — förhindrar permanent slutenhet genom att bevara tillit–utforskningsloopen genom krisen.' },
     { href: '/essays/the-optimum-that-depended-on-the-ruler', label: 'Notis — Det optimum som berodde på linjalen', digest: 'Ett beräkningsinriktat utforskande som började som en metafor och gav ett rent, förhandsregistrerat inre optimum för adaptiv eskalering under subsidiaritet — som sedan upplöstes under en robusthetskontroll som ändrade definitionen av koordinationskapacitet från händelseantalbaserad till magnitudbaserad. De överlevande fynden är smalare men skarpare: subsidiaritet minskar koordinationslagrets belastning endast när den mottagande nivån har verklig kapacitet (delegering utan kapacitet är sämre än ingen delegering), de tre arkitekturerna misslyckas med kvalitativt olika signaturer under samma stress, och matchningsgraden är ren aritmetik, inte emergent intelligens. Lärdomen är att "koordinationskapacitet" inte är en självklar storhet utan ett modelleringsval som kan vända på utslaget.' },
    ],
    synthesesItems: [
      {
        slug: 'we-see-you',
