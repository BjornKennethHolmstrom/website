...
The Resilience Dividend redirects existing EU structural fund allocations — primarily from the Common Agricultural Policy (CAP) and cohesion funds — toward localized economic security baselines in regions where the GSI identifies high informal care capacity and governance-tier mismatch.
...
1. **GSI diagnostic evidence:** The region has been assessed through the Parallel Diagnostic Phase (at Bronze tier minimum) and the OGT Matrix has identified governance-tier mismatch in at least one policy domain.
2. **Civic Value Registry participation:** The region has deployed the Registry and can demonstrate minimum viable data quality (Silver tier or above for at least two Knowledge Inclusion indicators).
3. **Member state authorization:** The member state has designated the region as a Regional Subsidiarity Zone under the Enhanced Cooperation agreement.
