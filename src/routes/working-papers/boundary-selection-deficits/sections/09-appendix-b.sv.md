...
**Figur B.1 – Stabilitet och M‑Δ-slingförstärkning vs. kopplingsstyrka (Scenarier a, b, c).**
![v14-stability-loopgain](/working-papers/images/boundary-selection-deficits/v14-stability-loopgain.png)
*Vänster panel:* Stabilitet \(S\) (högre värden indikerar bättre stabilitet) som en funktion av den globala kopplingsstyrkan \(\gamma\) för tre gränsscenarier. Scenario (a) — perfekt matchade gränser — upprätthåller hög stabilitet över hela intervallet för \(\gamma\), vilket bekräftar att M‑Δ-slingan är försumbar när jurisdiktionella perimetrar sammanfaller med de naturliga kopplingsklustren. Scenario (b) — Westfaliska slumpmässiga gränser — visar progressiv stabilitetsförsämring när \(\gamma\) ökar, där medianstabiliteten korsar in i negativt territorium vid ungefär \(\gamma = 0,20\). Scenario (c) — Sykes‑Picot-gränser — uppvisar den tidigaste och allvarligaste försämringen, där stabiliteten kollapsar vid \(\gamma \approx 0,10\). Gapet mellan kurvorna är den strukturella kostnaden för gränsmissanpassning: den stabilitetsmarginal som förloras genom att dra gränser som inte matchar den underliggande kopplingsstrukturen. *Höger panel:* Uppskattad M‑Δ-slingförstärkning \(\|\mathbf{M}\|\cdot\|\mathbf{\Delta}\|\) för samma scenarier. Den röda streckade linjen markerar enhetsförstärkningströskeln; när slingförstärkningen överstiger ett genererar regulatorns egna interventioner förstärkta återvändande störningar. Scenario (a) förblir säkert under ett. Scenario (b) närmar sig ett vid hög koppling. Scenario (c) korsar ett vid måttlig koppling, vilket bekräftar mekanismen bakom stabilitetsförsämringen: Sykes‑Picot-gränserna skapar aktivt strukturerad gränsöverskridande återkoppling som regulatorn inte kan observera.

**Figur B.2 – Adaptiv gränsförhandling vs. Westfalisk baslinje (Scenario d‑svep).**
![v14-adaptive-sweep](/working-papers/images/boundary-selection-deficits/v14-adaptive-sweep.png)
Medianstabilitet \(S\) som en funktion av kopplingsstyrkan \(\gamma\) för den Westfaliska baslinjen (Scenario b, orange) och scenariot med adaptiv omförhandling (Scenario d, magenta). Vid låga kopplingsstyrkor presterar de två arkitekturerna jämförbart. När \(\gamma\) ökar, överträffar adaptiv omförhandling väsentligt de statiska Westfaliska gränserna: genom att slå samman jurisdiktioner som upplever hög uppskattad gränsmissanpassning \(B_{\text{upp}}\) internaliserar den adaptiva arkitekturen delvis de spridningseffekter som försämrar den statiska arkitekturens prestanda. Emellertid är den adaptiva fördelen begränsad. Vid de högsta kopplingsstyrkorna kan inte ens adaptiv omförhandling helt sluta gapet till den perfekt matchade arkitekturen (Scenario a, visas inte), eftersom själva omförhandlingsprocessen introducerar latens \(\tau_{\text{just}}\) under vilken de gamla gränserna förblir i kraft. Detta är gränsjusteringskontrollproblemet: systemet måste omförhandla snabbare än kopplingsstrukturerna förändras, och när miljöförändringstakten överstiger omförhandlingsbandbredden halkar även en adaptiv arkitektur efter.

**Figur B.3 – Adaptiv omförhandlingsbana (Scenario d, enkel körning, \(\gamma = 0,20\)).**
![v14-adaptive-trajectory](/working-papers/images/boundary-selection-deficits/v14-adaptive-trajectory.png)
*Övre panel:* Genomsnittlig uppskattad gränsmissanpassning \(\bar{B}_{\text{upp}}\) över alla jurisdiktioner över tid. Den röda streckade linjen markerar omförhandlingströskeln \(B_{\text{trösk}} = 0,3\). När \(\bar{B}_{\text{upp}}\) överstiger denna tröskel initierar de jurisdiktioner med högst missanpassning fusionsförhandlingar. Sågtandsmönstret återspeglar processens cykliska natur: gränsmissanpassning ackumuleras när miljön förändras; omförhandling utlöses; missanpassning löses delvis genom jurisdiktionell fusion; cykeln återupptas. *Nedre panel:* Antal jurisdiktioner över tid. Den initiala Westfaliska konfigurationen av \(M=4\) jurisdiktioner konsolideras progressivt när hög-missanpassnings-jurisdiktioner slås samman. Banan illustrerar gränsförhandlingskontrollslingan: styrningsarkitekturen justerar sin egen perimeter som svar på observerade spridningskostnader, där justeringslatensen \(\tau_{\text{just}}\) avgör om konsolideringen springer före eller hamnar efter den föränderliga kopplingsstrukturen.
