...
Detta viker sig direkt in i Artikel XVI:s koppling snarare än att stå bredvid den. Källtermens lokalitet handlade om kapaciteten att *generera alternativ*; kedjelängd handlar om kapaciteten att *uppdatera regeln*; båda är en koppling – **egenskapen som gör ett system adaptivt (en källterm inuti loopen, en representation av dess egen regel) är samma egenskap som gör det redigerbart, eroderbart eller mutbart.** 

Det mellanliggande i den kopplingen – en regel som i princip är redigerbar men så kostsam att redigera att vanligt tryck lämnar den intakt – är den skyddsklass som Artikel XVI §6 lämnar öppen och undersöker beräkningsmässigt. Där öppnar sig den irreversibla fällan endast när utträde ur den sittande regeln är nästan utestängt, så en enbart kostsam `L_c ≥ 1`-länk är metastabil, inte absorberande: den avklingar tillbaka till ifrågasättbarhet så snart trycket lättar. Redigerbarhet är därför inte binär. Certifieringsgolvet fastställer det som inte alls kan avlägsnas; huruvida en i princip avlägsningsbar länk beter sig som avlägsningsbar beror på hur nära gränsen till utestängning dess omställningskostnad ligger. **[IP]**

L_c = 0 är gränsfallet: oredigerbarhet köpt genom att helt avstå från adaptivitet, den geografiska motsvarigheten till en fullständigt koncentrerad posterior som inte längre kan lära.
