...
**Figure B.1 – Phase diagram: basins of attraction in (L₀, T) space.**
![v15-phase-diagram](/working-papers/images/legitimacy-as-emergent-gain/v15-phase-diagram.png)
*Left panel:* Final legitimacy \(L(T=300)\) as a function of initial legitimacy \(L(0)\) (vertical axis) and the controller’s transparency level \(T\) (horizontal axis), under the built‑legitimacy parameter set. The green region at upper right is the high‑\(L\) basin: systems starting with sufficient initial legitimacy and maintaining adequate transparency converge to a stable high‑trust equilibrium. The red region at lower left is the low‑\(L\) attractor. *Right panel:* Probability of entering the legitimacy trap (defined as \(L\) falling below \(L_{\text{crit}}\) and failing to recover) for the same parameter sweep. The black contour marks the separatrix—the boundary between the basins. Systems initialised below and to the left of this contour are overwhelmingly likely to spiral into the trap. The figure makes visible the structural difference between the two legitimacy regimes: built legitimacy (shown) exhibits a broad high‑\(L\) basin; the borrowed‑legitimacy parameter set (not shown; see Section 4.2) produces a substantially narrower basin and a higher separatrix.
...
Six representative trajectories (distinguished by colour) that experienced a stochastic revelation event. *Top panel:* True state norm \(\|\mathbf{x}(t)\|\) (solid) and reported state norm \(\|\mathbf{x}_{\text{rep}}(t)\|\) (dashed). Before revelation, the reported state flatters the true state—the governed population perceives better outcomes than actually exist. After revelation (vertical dotted lines), the two converge. *Middle panel:* Legitimacy \(L(t)\). Apparent \(L\) is sustained by the suppressed reporting until the moment of revelation, at which point the betrayal penalty \(\gamma = 3.0\) is applied and \(L\) collapses catastrophically—falling far below the level that honest governance with the same underlying performance would have produced. *Bottom panel:* Cumulative hidden discrepancy \(E_{\text{betrayal}}(t)\). The discrepancy accumulates during the suppression period; revelation occurs when a stochastic hazard (governed by \(h=0.02\)) triggers. Longer suppression produces larger accumulated discrepancy, which produces a more severe collapse, as demonstrated by the parameter sweep in Figure B.4.
...
*Left panel:* Minimum post‑revelation legitimacy \(L_{\text{min}}\) as a function of the expected suppression duration (horizontal axis) and betrayal sensitivity \(\gamma\) (vertical axis). Long suppression combined with high \(\gamma\) produces the most severe collapses—\(L\) falls below 0.1, effectively destroying the governance system’s effective actuation and observation capacity. *Right panel:* Probability of entering the legitimacy trap after revelation. The black contour marks the 50% threshold. The figure identifies the structural conditions under which borrowing legitimacy is survivable (short suppression, low \(\gamma\), built‑legitimacy baseline) and those under which it is fatal.
